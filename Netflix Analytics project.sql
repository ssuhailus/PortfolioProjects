...
-- The project was written using the PostgreSQL dialect and utilized data from Netflix datasets.
...
(
    ...
    title varchar(58), 
    country varchar(74), 
    date_added datetime,  -- SQL Server uses datetime instead of timestamp
    release_year int, 
    rating varchar(5), 
    duration varchar(9), 
...
VALUES
    ...
    ('s17', 'Pedro de Echave García| Pablo Azorín Williams', NULL),
...
SELECT MAX(date_added) AS most_recent
from "netflix_titles_info";
...
select TOP 1 release_year, title
from "netflix_titles_info"
where type = 'Movie'
